...
> **Source of truth (verbatim).** This file is the calculation-engine team's
> integration contract, copied unchanged into this repo so that every Ralph
> iteration can read it offline. **Do not edit it to match our code — change our
> code to match it.** The component's adapter rules that map on-chain state onto
> this contract live in [`../ENGINE_CONTRACT.md`](../ENGINE_CONTRACT.md).
